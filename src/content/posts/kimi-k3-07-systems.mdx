...
flowchart TD
  subgraph 每个 rank 本地
    A[本地 token] --> B["算 M（累积转移，128×128/头）"]
    A --> C["算 S̃（从 0 出发的状态，128×128/头）"]
  end
  B & C --> D[一次 all-gather<br/>交换固定大小的片段]
  D --> E["rank i+1：从 S = 0 出发<br/>按顺序对前面每个片段做 S ← M_j S + S̃_j"]
  E --> F[得到精确的进入状态<br/>继续本地 chunkwise 计算]
```
